...
ご興味ありがとうございます! AgentLoom は **業界別レシピ** のコミュニティ駆動の蓄積を目指しています。

## 受け入れる Contribution の種類

### 1. 新しい業務フロー (`agentloom/flows/`)
特定の業務 (購買承認 / 障害対応 / コードレビュー …) に特化した
マルチエージェント・フローを `Flow` のサブクラスとして追加。

### 2. 新しいビジネススキル (`agentloom/skills/business/`)
...
### 4. LTM バックエンド (`agentloom/memory/backends/`)
...
```bash
git clone https://github.com/your-org/agentloom.git
cd agentloom
pip install -e ".[all]"
pytest                          # テスト実行
ruff check . && mypy agentloom  # 静的解析
```
