...
`hello@praxia.tools` への通知は (任意ですが) 歓迎 — こちらで宣伝協力できます。
...
以下のドメイン登録は不可:
- TLD で「praxia」を SLD として GenArch がプレゼンスを持つ領域 (`praxia.com`, `praxia.io`, `praxia.tools` 等は GenArch 意図的 reserve、未登録でも対象)
...
`trademark@praxia.tools` 宛 (または `trademark` ラベル付き GitHub Issue) に以下を含めて連絡:
...
以下の場合は `trademark@praxia.tools` に連絡:
...
| 用途 | 連絡先 |
|---|---|
| 許諾申請 | trademark@praxia.tools |
| 誤用報告 | trademark@praxia.tools |
| 一般質問 | hello@praxia.tools |
