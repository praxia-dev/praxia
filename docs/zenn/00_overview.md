...
### 13〜18: エンタープライズ運用機能 (NEW)
| # | 優位性 | 一言 |
|---|---|---|
| 13 | 管理者ユーザ CRUD | 作成/編集/削除/有効化/キーローテーション、全操作監査 |
| 14 | リソースアクセスポリシー (ACL) | 情報システム部向け Glob ベース allow/deny — 業界唯一 |
| 15 | 管理者データダウンロード | 監査ログ/ユーザ/メモリ/ポリシー を CSV/JSON でエクスポート、自身も監査される |
...
```bash
pip install "praxia[ui,connectors]"
