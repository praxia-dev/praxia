...
Praxia は「使うべき場所」を明確にしています。中堅〜大企業で、**シニアの暗黙知が個人の引き出しに留まっている** 組織に最適化されています。

| ペルソナ | 抱えている課題 | Praxia が刺さるポイント | 想定 1 年目効果 |
|---|---|---|---|
| **🏢 情報システム / プラットフォーム部門** (300〜5,000 人規模) | 全社 AI 展開で SSO / RBAC / 監査が paywall されない選択肢が欲しい。オンプレ要件もある | OIDC SSO・RBAC・ACL・ユーザ委譲 OAuth・監査ログが**全て OSS に内包**。同一コードを self-hosted で運用可能 | 100 ナレッジワーカー × 純利益 **$1.25M / 年**、完全な監査トレイル、シート単価ロックイン無し |
| **🏗️ エンジニアリング / プロダクト VP** (50〜500 人スコープ) | シニアアーキテクトのレビュー負荷がボトルネック。ジュニア PM の立ち上がり 12〜18 ヶ月 | DesignSkill (DRAGON) + Sleep-time Consolidation で「シニア X のレビュー観点」を組織知化。Markdown + git は既存 PR ワークフローに馴染む | シニア負荷 **16h/週 → 4h/週**、ジュニア立ち上がり **6〜9 ヶ月**、NFR カバー範囲 5-7 → **15-20 軸** |
| **⚖️ 法務 / コンプライアンス責任者** (規制業界) | 月 50〜100 件の契約書レビューが 2〜3 名に集中。期限プレッシャーで重要リスクを見落とす | LegalSkill (RACE) + read-only メモリモード + ユーザ委譲 OAuth で連携先 ACL 尊重 + 全操作監査。Apache 2.0 でソース監査可能 | 1 件あたり **60-90 分 → 10-15 分**、処理量 **50-80件/月 → 200-300件/月**、重大リスク見落とし **5-10% → 1-2%** |
| **🧪 OSS / 研究開発インテグレータ** | Mem0 / LangGraph / 自前ベクトル DB の上に独自ドメインエージェントを作りたいが、認証 / メモリ循環 / ダッシュボード / エクスポータを 0 から書きたくない | 7 拡張ポイント (各 ~50 LoC) — connector / memory backend / parser / exporter / OAuth provider / skill / flow。ライブラリ利用も `praxia serve` での HTTP 利用も可能。LangGraph 内ノードに埋込可 | 1 ヶ月目: ドメインスキル PR 完了、独自コネクタ pip 配布可、メモリ循環稼働。**0 から書くより 3 週間先行** |

詳細な業務別 Before/After: [docs/use-cases.md](https://github.com/genarch/praxia/blob/main/docs/use-cases.md)

## なぜ OSS であることが効くのか

「エンタープライズ機能のために有償プランへ」というモデルではなく、**Apache 2.0 の同梱パッケージで全機能** が使えることが Praxia の核です:

- **SSO / RBAC / 監査ログが paywall されない** — Google / Microsoft / Okta / GitHub / Keycloak の OIDC が OSS に組込済。マルチエージェント FW としては珍しく、エンタープライズ FW としても珍しい
- **データ形式がロックインされない** — Layer 4 は git 管理 Markdown / Layer 3 は JSONL / Layer 1 は backend ネイティブ。離脱コストゼロ
- **全コード読める** — 監査・セキュリティチーム・お客様にソースを開示可能
- **複数 LTM のアンサンブル**運用が標準機能 — 商用エージェント基盤は単一バックエンドにロックインしがちですが、Praxia は最初から複数並列 + RRF 融合を提供
- **ユーザ委譲 OAuth** で連携先の ACL を尊重 — alice が pull すると Box の alice 用 ACL が適用。サービスアカウント運用ではユーザ間でデータ漏れる
- **エアギャップ運用可能** — `PRAXIA_LOCAL_MODEL=gemma` + Ollama + `backend=json` で完全オンプレ。クラウド利用と同一コード
- **本番グレードの OAuth + KMS 暗号化が OSS** — multi-worker 安全な state cache + 5 KMS アダプタ (AWS / Azure / GCP / Vault / local)。商用エージェント基盤では有償化されがちな機能を OSS で提供
- **A/B 実験 + 品質評価が同梱** — プロンプト変更を実ユーザで決定論的に検証。CI で LLM 出力品質のデグレを自動検知
...
| # | 優位性 | 一言 |
|---|---|---|
| 23 | **複数 LTM 同時利用 + 動的ルーティング** | RRF / Union / Intersection / Weighted / LLM rerank の 5 戦略 + ルール / LLM ルータ (英日両対応) |
| 24 | **メモリモード切替 (accumulate / read_only)** | 機微情報利用時に「足跡を残さない」運用に対応、admin がロック可 |
| 25 | **管理者向け LTM ポリシー** | バックエンドの全社固定 / 許可リスト / モードロック / ロール別強制 |
| 26 | **出力エクスポータ (HTML / PPTX / DOCX / MD / JSON)** | `OutputFormatSkill` が日本語ヒント (「パワポで」) を自動判定 |
| 27 | **Gemma 対応** | `gemma` / `gemma-2b/9b/27b` (Ollama) / `gemma-cloud` (Vertex AI) |
| 28 | **2 つのデプロイモード** | フルスタック (`praxia ui`) または HTTP バックエンドのみ (`praxia serve` FastAPI) |
| 29 | **正式設計仕様書 (英日)** | 基本設計 / I/F / 詳細設計の 3 文書を 2 言語で整備 |

### 30〜34: 本番運用・品質ガード + 自律エージェント (NEW)
| # | 優位性 | 一言 |
|---|---|---|
| 30 | **KMS / HSM ベースのトークン暗号化** | AES-GCM envelope encryption + 5 アダプタ (`local` / `aws` / `azure` / `gcp` / `vault`)、マスターキーは KMS / HSM 内 |
| 31 | **本番 OAuth コールバック (HTTP)** | `praxia serve` で `/api/v1/oauth/{provider}/{start,callback,status}` を提供。multi-worker 安全な state cache |
| 32 | **A/B 実験フレーム** | 決定論的アサインメント (SHA-256) + アウトカム計測 + 暫定 winner 検出。CLI + SDK 両対応 |
| 33 | **LLM 出力品質評価フレーム** | `tests/llm_eval/` で実 LLM 呼出し + ベースライン比較。CI で品質デグレを自動検知 (5pt 超ドロップで failure) |
| 34 | **自律エージェント (ClaudeCode 同等)** | LLM 駆動のツール使用ループで個人メモリ・組織メモリ・凍結層・スキル・コネクタを自発的に活用。ACL チェックと監査ログ込み。`praxia agent run` / MCP メタツール対応 |

### 自律エージェント (AutonomousAgent) — 自発的にツールを選んで動く

`praxia.agent.AutonomousAgent` は ClaudeCode 同様の **LLM 駆動ツール使用ループ** を Praxia の各レイヤ上で実行します。明示的に「メモリ検索 → スキル実行 → コネクタ pull」と書かなくても、エージェントが LLM の判断で必要なツールを選び、最終回答を出すまでループします。
...
agent = AutonomousAgent(
    user_id="alice", role="member", org_id="acme",
    llm=LLM("claude"), max_steps=10,
)
result = agent.run("Acme との今四半期の営業状況を整理して提案書ドラフトを作成して")
print(result.final_text)
for tc in result.tool_calls:
    print(f"- {tc.name}({tc.arguments_text[:60]}) -> ok={tc.ok}")
```

組込みツール 11 種類 (個人メモリ検索 / 組織メモリ検索 / 凍結層検索 / スキル一覧 / スキル実行 / コネクタ pull / 事実記録 / 最終回答 など) — 全ツール呼出を `auth.audit` に記録、`pull_from_connector` は `auth.policies.require()` で ACL チェック (拒否時はエージェントに伝達してループ継続)、`record_fact` は `read_only` モードで no-op。

CLI: `praxia agent run "<task>"`、MCP メタツール `autonomous_agent` で Claude Desktop / Cursor から委任可能。
...
LLM("gemma")         # Google Gemma 9B (Ollama ローカル)
LLM("gemma-cloud")   # Google Gemma 27B (Vertex AI 経由)
LLM("qwen")          # Alibaba Qwen Max (DashScope API)
LLM("qwen-local")    # Qwen ローカル (Ollama 経由)
LLM("openai/gpt-4o") # 任意の LiteLLM 形式
```

**完全オフライン運用** も可能 (Qwen-local / Gemma + JSON memory backend)。
...
## 出力形式の自動判定 — HTML / PPTX / DOCX / MD / JSON

スキルは既定で Markdown を返します。ユーザの自然言語ヒントから最適な配信
形式を判定して変換するのが `OutputFormatSkill`:

```python
from praxia.skills.output_format import OutputFormatSkill
from praxia.skills.business import InvestmentSkill

md = InvestmentSkill().run("Acme Corp の Q3 レビュー")
result = OutputFormatSkill().deliver(md, user_request="このレポートをパワポで")
# result.format == "pptx"
# result.bytes  → ファイル保存 / HTTP 配信 / コネクタ push
```

英語と日本語の両方を判定 (「パワポで」「Word の文書」「html」「JSON で」等)。
CLI でも:
```bash
praxia export report.md slides.pptx --title "Q3 レビュー"
praxia export report.md report.html
```

カスタム形式 (LaTeX / Confluence Storage 等) は entry-point で追加可能。

## 2 つのデプロイモード — フルスタック / バックエンドのみ

| モード | 起動 | 向くケース |
|---|---|---|
| **A. フルスタック** | `praxia ui` (Streamlit) | 内製チームが最短ルートで使う |
| **B-1. SDK 埋込** | 自前 Python サービスに `import praxia` | 既存の Python バックエンドを拡張 |
| **B-2. HTTP サービス** | `praxia serve` (FastAPI) + 自前フロント | Next.js / モバイル / Slack 等の Python 以外フロント |

両モードで認証・メモリ・スキルは完全共通。差は **フロントを誰が持つか** だけ。

```bash
# フルスタック
praxia ui --port 8501

# バックエンドのみ (8 endpoint /api/v1)
pip install "praxia[server]"
praxia serve --host 0.0.0.0 --port 8000 --cors-origin https://your-frontend.example
```

詳細手順 + 本番チェックリスト: [docs/deployment-modes.ja.md](https://github.com/genarch/praxia/blob/main/docs/deployment-modes.ja.md)

## メモリモード切替 — accumulate / read_only

機微情報の利用時に「足跡を残さない」運用が必要なら、ユーザ毎に `read_only`
モードへ切替可能。記録 API (`record_episode` 等) は no-op になりますが、検索
は通常動作:

```bash
praxia memory mode --user-id alice read_only      # 書込無効化
praxia memory mode --user-id alice accumulate     # 戻す
praxia memory show --user-id alice                # 解決済み設定確認
```

管理者は全社 / 特定ロールに対してロック可能:
```bash
...
優先順位: admin enforced > 呼出引数 > user pref > admin default。
