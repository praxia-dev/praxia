...
| 音声 I/O (STT + TTS) | |
| 画像入力 (チャット / 自律エージェントの Vision 添付) | |
| 会話履歴の永続化 (再開 / 改名 / 削除) | |
| 任意 FastAPI HTTP サーバ (`praxia serve`) | |
| KMS / HSM ベースの OAuth トークン暗号化 (5 アダプタ) | |
| 本番 OAuth コールバック ハンドラ (multi-worker 安全) | |
| A/B 実験フレーム | |
| LLM 出力品質評価フレーム | |
...
| 1 | `praxia.core.llm` | マルチプロバイダ LLM クライアント (LiteLLM) — 27 のフレンドリエイリアス (Anthropic / OpenAI / Google / DeepSeek / Mistral / xAI / Cohere / Perplexity / Qwen / Llama / Gemma / Phi 等)。`LLMResponse.tool_calls` で関数呼出構造を露出 (自律エージェントループが利用) |
| 2 | `praxia.core.agent` / `flows` / `skills` | エージェント + フロー + スキルレジストリ |
| 2a | `praxia.agent` | **自律エージェント** — Praxia スタック全体 (メモリ/スキル/コネクタ/凍結層) を対象に LLM 駆動でツール使用ループを実行。組込ツール 11 種、ACL ゲート、監査ログ完備。CLI コマンド + MCP メタツールとして公開 |
...
| 8 | `praxia.experiments` | A/B 実験 — variant アサインメント + 結果集計 |
| 9 | `tests/llm_eval` | LLM 出力品質評価フレーム (CI ガード) |
...
Layer 1 のバックエンド選択と蓄積モードは `MemoryAdminPolicy` 経由で統制 (Streamlit Admin → Settings → メモリポリシーまたは `praxia admin memory-policy-*` CLI で管理者が編集)。`MemoryUserPreference` SDK クラスはレゾルバ内に残置するが、UI からの書き込み経路は意図的に存在せず、メモリ設定は管理者専管事項。詳細: `praxia.memory.policy`。
