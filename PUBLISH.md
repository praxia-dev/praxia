...
URL: `https://praxia.tools/`

カスタムドメイン (`praxia.tools` 等) を持っているなら、Cloudflare DNS で 1 クリック設定可能。
...
`praxia.tools` を取得済 (本リポジトリ運用ドメイン)。

DNS は Cloudflare に集約するのがベスト (無料 + DDoS 対策付き) — Cloudflare Pages との統合も自動。
...
| Day 30 〜 (専用ドメイン) | `praxia.tools` (CF DNS) | 年 $12 | 低 |
