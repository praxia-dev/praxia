...
詳細な業務別 Before/After: [docs/use-cases.md](https://github.com/praxia-dev/praxia/blob/main/docs/use-cases.md)
...
| 34 | **自律エージェント (LLM 駆動)** | LLM 駆動のツール使用ループで個人メモリ・組織メモリ・凍結層・スキル・コネクタを自発的に活用。ACL チェックと監査ログ込み。`praxia agent run` / MCP メタツール対応 |

### 自律エージェント (AutonomousAgent) — 自発的にツールを選んで動く

`praxia.agent.AutonomousAgent` は **LLM 駆動のツール使用ループ** を Praxia の各レイヤ上で実行します。明示的に「メモリ検索 → スキル実行 → コネクタ pull」と書かなくても、エージェントが LLM の判断で必要なツールを選び、最終回答を出すまでループします。
...
詳細手順 + 本番チェックリスト: [docs/deployment-modes.ja.md](https://github.com/praxia-dev/praxia/blob/main/docs/deployment-modes.ja.md)
