...
_T: dict[str, dict[str, str]] = {
    ...
    "admin.settings.runtime_h": {
        "en": "Runtime: LLM model + memory backend (current process only)",
        "ja": "ランタイム: LLM モデル + メモリバックエンド (現プロセスのみ)",
        "zh-CN": "运行时: LLM 模型 + 记忆后端 (仅当前进程)",
        "ko": "런타임: LLM 모델 + 메모리 백엔드 (현재 프로세스 전용)",
        "es": "Runtime: modelo LLM + backend de memoria (solo proceso actual)",
        "fr": "Runtime : modèle LLM + backend mémoire (processus actuel uniquement)",
        "de": "Laufzeit: LLM-Modell + Memory-Backend (nur aktueller Prozess)",
        "pt-BR": "Runtime: modelo LLM + backend de memória (apenas processo atual)",
    },
    "admin.settings.runtime_intro": {
        "en": "Session-scoped override. Lost on Streamlit restart unless persistent **Memory policy** below sets the same default. Use this to test a different model/backend without committing to it.",
        "ja": "セッション内のみの一時上書き。Streamlit を再起動すると失われます (下の **メモリポリシー** で恒久化していない限り)。本番固定せずにモデルやバックエンドを試すための枠。",
        "zh-CN": "仅当前会话生效。Streamlit 重启后失效（除非下方 **内存策略** 中已永久设置）。用于试用不同模型/后端而不固化。",
        "ko": "세션 한정 오버라이드. Streamlit 재시작 시 사라집니다 (아래 **메모리 정책** 에서 영구화하지 않은 한). 모델/백엔드를 임시로 시험할 때 사용.",
        "es": "Override solo en la sesión actual. Se pierde al reiniciar Streamlit (a no ser que la **política de memoria** de abajo lo fije). Útil para probar otro modelo/backend sin fijarlo.",
        "fr": "Override de session uniquement. Perdu au redémarrage de Streamlit (sauf si la **politique mémoire** ci-dessous le fixe). Pour tester un autre modèle/backend sans l'engager.",
        "de": "Sitzungsbezogenes Override. Beim Neustart von Streamlit verloren (außer die **Memory-Richtlinie** unten setzt den gleichen Standard). Zum Testen ohne festzuschreiben.",
        "pt-BR": "Override apenas na sessão. Perdido ao reiniciar o Streamlit (exceto se a **política de memória** abaixo fixar). Útil para testar outro modelo/backend sem fixar.",
    },
    "admin.settings.runtime_intro": {
