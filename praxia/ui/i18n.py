...
_T: dict[str, dict[str, str]] = {
    ...
    "login.api_key_help": {
        ...
        "ja": "`praxia user create` で発行した API キーを貼り付け。キー無しなら単一ユーザの開発モードになります。",
        "zh-CN": "粘贴通过 `praxia user create` 发出的 API 密钥。无密钥则进入单用户开发模式。",
        "ko": "`praxia user create` 로 발급한 API 키를 붙여넣으세요. 키 없으면 단일 사용자 개발 모드.",
        "es": "Pega una clave API emitida con `praxia user create`. Sin clave, modo dev/usuario-único.",
        "fr": "Collez une clé API émise via `praxia user create`. Sans clé, mode dev/mono-utilisateur.",
        "de": "API-Key einfügen, der via `praxia user create` ausgestellt wurde. Ohne Key: Einzelnutzer-Dev-Modus.",
        "pt-BR": "Cole uma chave API emitida via `praxia user create`. Sem chave, modo dev/usuário-único.",
    },
    "login.submit": {
        "en": "Sign in",
        "ja": "サインイン",
